...
Nobody disputes that the harness affects the benchmark score. OpenAI, the UK AI Security Institute, and METR have all said so publicly. What nobody has measured is HOW the harness affects it: which pieces of the harness are doing the work, whether those pieces help/hurt each other, and whether any of that stays the same when you swap in a different model. This repo runs that measurement.
...
- [The three things we test](#the-three-things-we-test)
...
The problem is that every one of those results depends on the whole harness and not the specific contributions within it. If the harness is swapped, the number changes, but we don't know exactly which single component was responsible, or whether two components together do more than the sum of what they do separately. This gap is what this project aims to fill.
...
If everything on the right comes out flat (credit spread evenly, rankings transfer perfectly, no ranking flips), that's still worth reporting. It would mean standardized harnesses are unbiased and cheap evaluations can be trusted, which is also a useful outcome to report.

## The three things we test
...
**2. The ranking may not transfer. This is the one we care most about.**
...
**3. Head to head comparisons can flip.**

Model A beats model B with no scaffolding, then loses once both are fully scaffolded. If that happens, then the verdict of a "controlled comparison" depends on a harness decision.
...
Two of the original six components aren't in that sweep. `retrieval` was built (BM25 over a Wikipedia corpus) and then dropped. Three real bugs plus an untested relevance question made it the least trustworthy component in the codebase, so it's commented out in `elicit_task.py` instead of deleted. `best_of_n` was never built. `chain_of_thought` (`cot`) exists as a toggle but sits outside this sweep. It was measured in an earlier, separate bare/cot/critique/cot+critique ablation (`run_ablation.py`) on both suites; the current interaction study attributes credit to the other four toggles only.
...
|---|---|---|---|
| Tool use (`tool_use`) | augments | Active | Lets the model call a sandboxed Python tool. Capped at a 30s per-call timeout and an identical-call repeat guard, added after a runaway tool-call loop on one model burned 15-40x the tokens of every other config in a sweep |
...
| Best-of-N (`best_of_n`) | reveals | Never built (deferred) | Generate N answers, pick one by voting or by judge |
...
All three models in the current sweep are complete: `Qwen2.5-72B-Instruct`, `Qwen3-32B`, `gemma-3-27b-it` (MATH intermediate_algebra, DeepInfra). Full detail, including a data-integrity bug that was found and fixed partway through this analysis, is in [`writeup/findings.md`](writeup/findings.md).

**Credit and interaction.** On `Qwen2.5-72B-Instruct`, `multi_critic` alone (+0.161, p<0.0001) accounts for 43% of the entire four-component lift (bare 0.263 to full-stack 0.636, +0.373 total). On `Qwen3-32B`, the whole four-component harness barely moves accuracy at all (+0.009 total, inside noise). On `gemma-3-27b-it`, `critique` and `planning` carry the lift (+0.172 total) while `multi_critic` and `tool_use` contribute close to nothing net. Three models, three different concentration patterns, and no single component is positive and significant on all three (`critique` and `planning` come closest, but even those flip sign on at least one model).

**Transfer across models.** Ranking the four components by main-effect size gives Spearman ρ = 0.20 between `Qwen2.5-72B-Instruct` and `Qwen3-32B`, ρ = 0.20 between `Qwen3-32B` and `gemma-3-27b-it`, and **ρ = −0.60 between `Qwen2.5-72B-Instruct` and `gemma-3-27b-it`**. None of these are individually significant (n=4 components is low power for a correlation), but a negative rank correlation between two of the three models is a real transfer-failure signal on its own. The clearest example: `multi_critic` looked like the single most trustworthy finding in this project with two models measured (positive on both, largest effect, immune to a validity confound found elsewhere in the data). It is not significant on the third model (p=0.42, point estimate near zero). That reversal is itself the finding, not a footnote, and it's the exact failure mode this project exists to catch.

**Flipped comparisons.** Bare, the ranking is `Qwen3-32B` (0.564) > `Qwen2.5-72B-Instruct` (0.263) > `gemma-3-27b-it` (0.208). Fully scaffolded, it's `Qwen2.5-72B-Instruct` (0.636) > `Qwen3-32B` (0.573) > `gemma-3-27b-it` (0.380). The top two swap places depending on how much harness is applied. One caveat that still applies to the bare-condition ranking specifically: a spot-check of `Qwen3-32B`'s bare-condition transcripts found visible chain-of-thought leaking into 8 of 12 sampled answers despite the system prompt saying not to show its work, which likely inflates its bare score (open item, not yet checked on the other two models, see `writeup/findings.md`).
...
- We are not claiming to have discovered that the harness matters, since that has been established. The contribution is the breakdown and the transfer result.
...
- Results are specific to the task suites and model families tested. Whether they generalize further has not yet been checked.
